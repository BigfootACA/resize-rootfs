...
string FormatSize(uint64_t val,const char**units,size_t blk){
	char buff[256];
	if(!format_size_ex(buff,sizeof(buff),val,units,blk))return "";
	return buff;
}

string FormatSizeFloat(uint64_t val,const char**units,size_t blk,uint8_t dot){
	char buff[256];
	if(!format_size_float_ex(buff,sizeof(buff),val,units,blk,dot))return "";
	return buff;
}
string FormatSizeHz(uint64_t val){
	return FormatSize(val,size_units_hz,1000);
}

string FormatSizeFloatHz(uint64_t val,uint8_t dot){
	return FormatSizeFloat(val,size_units_hz,1000,dot);
}

string SizeString(uint64_t val){
	return format("{} ({} bytes)",FormatSize(val),val);
}

string SizeStringFloat(uint64_t val){
	return format("{} ({} bytes)",FormatSizeFloat(val),val);
}
